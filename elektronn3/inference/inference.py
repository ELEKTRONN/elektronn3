...
Tensor
    ...
    final_crop_slice = _extend_nc([slice(int(l), int(h)) for l, h in zip(crop_low_corner, crop_high_corner)])
    ...
    for tile_pos in pbar:
        ...
        inp_slice = _extend_nc([slice(int(l), int(h)) for l, h in zip(inp_low_corner, inp_high_corner)])
        # Output slice without overlap (this is the region where the current
        #  inference result will be stored)
        out_slice = _extend_nc([slice(int(l), int(h)) for l, h in zip(out_low_corner, out_high_corner)])
